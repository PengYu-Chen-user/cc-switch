...
默认：`~/.kimi-code/`（可用 `KIMI_CODE_HOME` 重定位）。

### 关键文件

```
~/.kimi-code/
├── config.toml       # 主配置（供应商、模型、default_model）
├── mcp.json          # MCP 服务器（mcpServers）
├── AGENTS.md         # 系统提示词
├── skills/           # Skills 目录
└── sessions/         # 会话记录
```
...
CC Switch 写入 `[providers.<key>]`、`[models."<alias>"]` 以及顶层 `default_model`，并保留无关表。
...
| `providers.<key>.api_key` | API Key（仅从此文件读取） |
...
CC Switch 写入 `llm-pi-ai.providers.<id>`，并让 `agent-default-model` 指向它。
