...
Default: `~/.kimi-code/` (relocatable via `KIMI_CODE_HOME`).

### Key Files

```
~/.kimi-code/
├── config.toml       # Main configuration (providers, models, default_model)
...
CC Switch writes `[providers.<key>]`, `[models."<alias>"]`, and the top-level `default_model`, preserving unrelated tables.
...
| Field | Description |
|-------|-------------|
| `providers.<key>.type` | `kimi`, `anthropic`, `openai`, `openai_responses` |
| `providers.<key>.base_url` | API endpoint |
| `providers.<key>.api_key` | API key (read only from this file) |
...
Default: `~/.dsh/` (relocatable via `DSH_HOME`).
...
CC Switch writes `llm-pi-ai.providers.<id>` and points `agent-default-model` at it.
...
| Field | Description |
|-------|-------------|
| `llm-pi-ai.providers.<id>.api` | `openai-completions`, `openai-responses`, `anthropic-messages` |
| `llm-pi-ai.providers.<id>.baseURL` | API endpoint |
| `llm-pi-ai.providers.<id>.apiKeyEnv` | Credential reference name |
| `agent-default-model` | Active route (`provider` + `model`) |

The API key is stored in `.credentials.yaml`:
