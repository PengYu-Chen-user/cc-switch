...
### The All-in-One Manager for Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes Agent, Kimi Code CLI & DeepSeek Harness

> **This fork** adds first-class support for **Kimi Code CLI** (`kimicode`, Moonshot AI) and **DeepSeek Harness** (`dsh`, DeepSeek AI) on top of upstream CC Switch.
...
Modern AI-powered coding relies on tools like Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes, Kimi Code CLI, and DeepSeek Harness — but each has its own configuration format. Switching API providers means manually editing JSON, TOML, or `.env` files, and there is no unified way to manage MCP and Skills across multiple tools.
...
- **One App, Ten Tools** — Manage Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes, Kimi Code CLI, and DeepSeek Harness from a single interface
...
This fork extends upstream with two additional tools, both managed in **switch mode** (pick a provider → it becomes the default model):

### Kimi Code CLI (`kimicode`)

Moonshot AI's terminal coding agent (`kimi`, npm `@moonshot-ai/kimi-code`).

- **Provider config**: `~/.kimi-code/config.toml` (or `$KIMI_CODE_HOME/config.toml`). CC Switch writes `[providers.<key>]` + `[models."<alias>"]` and sets `default_model`, merging non-destructively so `thinking`, `loop_control`, and OAuth-managed providers are preserved.
- **Provider types**: `kimi`, `anthropic`, `openai`, `openai_responses`.
- **MCP**: servers are synced to `~/.kimi-code/mcp.json` (`mcpServers`).
- **Skills / Prompts**: `~/.kimi-code/skills/`, `~/.kimi-code/AGENTS.md`.
- **Sessions**: browse/restore sessions from `~/.kimi-code/sessions/`.

### DeepSeek Harness (`dsh`)

DeepSeek AI's plugin-based agent harness (`dsh`, npm `@deepseek-ai/dsh`).

- **Provider config**: `~/.dsh/settings.yaml` (or `$DSH_HOME/settings.yaml`). CC Switch writes `llm-pi-ai.providers.<id>` and selects it via `agent-default-model`.
- **Credentials**: the API key is written to `~/.dsh/.credentials.yaml` under `refs.<apiKeyEnv>`; `settings.yaml` only holds the reference.
- **Protocols**: `openai-completions`, `openai-responses`, `anthropic-messages`.
- **Skills / Prompts**: `~/.dsh/skills/`, `~/.dsh/AGENTS.md`.
- **Sessions**: browse sessions from `~/.dsh/sessions/` (JSONL, zstd-aware).
- **MCP**: deferred — DSH's MCP wiring uses an unstable Cordis patch format, so the app is registered but not yet projected.
...
- **10 supported tools, 50+ presets** — Claude Code, Claude Desktop, Codex, Gemini CLI, Grok Build, OpenCode, OpenClaw, Hermes, Kimi Code CLI, DeepSeek Harness; copy your key and import with one click
...
CC Switch supports ten tools: **Claude Code**, **Claude Desktop**, **Codex**, **Gemini CLI**, **Grok Build**, **OpenCode**, **OpenClaw**, **Hermes**, **Kimi Code CLI** (`kimicode`), and **DeepSeek Harness** (`dsh`). Each tool has dedicated provider presets and configuration management.
